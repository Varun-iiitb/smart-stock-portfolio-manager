...
def data (ticker_symbol):
    ...
    if (week_later_pred > today_pred_1):
        if (((week_later_pred-today_pred)/today_pred)*100 > 10):
            output_2 = ("The price is projected to increase over the next week. It may be a good opportunity to consider buying the stock now.")        
        else:
            output_2 = ("There is no significant price increase expected in the coming week. The stock may show potential later, so it is advisable to wait before making a purchase.")
    else:
        if (((today_pred - week_later_pred)/today_pred)*100 > 10):
            output_2 = "The stock price is projected to decline significantly over the next week. It may be advisable to sell any holdings at this time."        
        else:
            output_2 = "There is no significant decline expected in the coming week. It may be wise to wait before making a move."
    ...
    fig.write_html("C:/Users/varun/tradesight_app/backend/prediction.html")
